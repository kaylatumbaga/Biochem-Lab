...
bradford <- read.table("bradford.dat", header=TRUE)
calibration <- lm(Concentration~Absorbance, data=bradford)
...
EnzymeStock <- set_units(c(0.1,predict(calibration, newdata=list(Absorbance=0.400))*4),mg/mL)
EnzymeStockM <- ;
...
|Substrate|k~cat~ (`r deparse_unit(Ldopakcat)`)|K~M~ (`r deparse_unit(LdopaKm)`)|k~cat~/K~M~ (`r deparse_unit(LdopaSC)`)|
|------|--------|---------|-----------|
|L-Dopa|`r drop_units(Ldopakcat)`|`r drop_units(LdopaKm)`|`r drop_units(LdopaSC)`|
|D-Dopa|`r drop_units(Ddopakcat)`|`r drop_units(DdopaKm)`|`r drop_units(DdopaSC)`|

Table: (\#tab:substrate) Put a more descriptive caption for this table here.
